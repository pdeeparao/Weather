...
class DetailsAdapter @Inject constructor() : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    class DetailsViewHolder(binding: ItemDetailCityBinding) : ViewHolder(binding.root) {
        ...
        fun bind(weatherDataResource: Resource<WeatherData>, pos: Int) {
            ...
            if (currWeather == null) {
                ...
                binding.currTemp.text = itemView.resources.getString(
                    R.string.temperature,
                    currWeather.main.temp.toString()
                )
                binding.loTemp.text = itemView.resources.getString(
                    R.string.temperature,
                    currWeather.main.low.toString()
                )
                binding.hiTemp.text = itemView.resources.getString(
                    R.string.temperature,
                    currWeather.main.high.toString()
                )
                binding.wind.text =
                    itemView.resources.getString(R.string.wind, currWeather.wind.speed.toString())
                binding.rain.text = itemView.resources.getString(
                    R.string.rain,
                    currWeather.rain?.hourly?.toString() ?: 0
                )
                binding.cloudiness.text = itemView.resources.getString(
                    R.string.cloudiness,
                    currWeather.clouds.all.toString()
                )
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val binding =
            ItemDetailCityBinding.inflate(LayoutInflater.from(parent.context), parent, false)
